...
(function ($) {
  ...
  var run_grid_range_change = function() {
    ...
    if (current_range == 4) { // large desktop
      $('.agenda-type').addClass('hni-grid').data({paddingTop: 1}); // do not remove - nth-child overider.
      $('.page-magazines .agenda-magazine').data({paddingTop: 1});
      $('.agenda-magazine:nth-child(-n+4) > .agenda-type, .agenda-event:nth-child(-n+4) > .agenda-type').data({paddingTop: 0});
      $('.page-magazines .agenda-magazine:nth-child(-n+4)').data({paddingTop: 0});
      $('.agenda-magazine.agenda-event .time-date-wrapper').addClass('dotdotdot').data({minHeight: 0, height: 2});
      $('.agenda-magazine.agenda-event .eventtitle').addClass('dotdotdot').data({minHeight: 0, height: 2.5});
      $('.magazine-event .eventtitle').addClass('dotdotdot').data({minHeight: 0, height: 2.5});
      $('.agenda-event, .agenda-magazine').data({height: 18});
      $('.footer-wrapper .col1:nth-child(2) > .footer-col:nth-child(2)').addClass('hni-grid').data({paddingTop: 0});
      $('.hni-header-home').addClass('hni-grid').data({marginBottom: 1, marginTop: 1});
      $('.hni-header__vertical-menu').removeClass('hni-grid').data({paddingBottom: 0});
      $('#agenda-filter-form').addClass('hni-grid').data({marginTop: 0});
      $('#agenda-filter-form .agenda-datepicker-label').addClass('hni-grid').data({paddingTop: 0.7});
      $('#agenda-filter-form .form-item-when-date-exact img').addClass('hni-grid').data({top: 0.7});
      $('.agenda-header').addClass('hni-grid').data({paddingTop: 0});
      $('.agenda-header-content').addClass('hni-grid').data({paddingBottom: 0});
      $('.agenda-event .event-image').addClass('hni-grid').data({height: 5});
      $('.agenda-magazine .agenda-magazine-cover img').addClass('hni-grid').data({height: 5});
      $('.press-item').addClass('hni-grid').data({paddingBottom: 3});
    }

    if (current_range == 3) { // 1024 - 1199
      $('.agenda-event, .agenda-magazine').data({height: 22});
      $('.agenda-type').addClass('hni-grid').data({paddingTop: 1}); // do not remove - nth-child overider.
      $('.page-magazines .agenda-magazine').data({paddingTop: 1});
      $('.agenda-magazine:nth-child(-n+3) > .agenda-type, .agenda-event:nth-child(-n+3) > .agenda-type').data({paddingTop: 0});
      $('.page-magazines .agenda-magazine:nth-child(-n+3)').data({paddingTop: 0});
      $('.agenda-header, .agenda-header-content').removeClass('hni-grid').removeData( "height" ).css('height', 'auto');
      $('.agenada-header-content').addClass('hni-grid').data({paddingTop: 1});
      $('.footer-wrapper .col1:nth-child(2) > .footer-col').addClass('hni-grid').data({marginBottom: 0, paddingBottom: 0});
      $('.footer-wrapper .col1:nth-child(2) > .footer-col:nth-child(2)').addClass('hni-grid').data({paddingTop: 1});
      $('.hni-header-home').addClass('hni-grid').data({marginBottom: 1, marginTop: 1});
      $('#agenda-filter-form').addClass('hni-grid').data({marginTop: 0}).css('overflow', '');
      $('.agenda-header-content').addClass('hni-grid').data({paddingBottom: 0.5});
      $('.agenda-event .event-image').removeClass('hni-grid').removeData( "height" ).css('height', 'auto');
      $('.page-magazines .agenda-magazine > .agenda-event-body').addClass('hni-grid').data({paddingLeft: 0, paddingRight: 0});
      $('.press-item').addClass('hni-grid').data({paddingBottom: 5});
     }

    if (current_range < 3) {
      $('.agenda-magazine .agenda-magazine-cover img').removeClass('hni-grid').removeData( "height" ).css('height', 'auto');
    }

    if (current_range > 2) {
      $('.page-agenda .agenda-magazine .event-body-teaser, .page-agenda .agenda-event .event-body-teaser').addClass('hni-grid').data({height: 3.5}).css('height','');
      $('.footer-wrapper .col1:nth-child(2) > .footer-col:nth-child(2) > .footer-col-title').addClass('hni-grid').data({paddingBottom: .25});
    }

    if (current_range < 2) {
      $('.footer-wrapper .col1:nth-child(2) > .footer-col:nth-child(2) > .footer-col-title').addClass('hni-grid').data({paddingBottom: 0});
    }

    if (current_range == 2) { // 768, 1023
      $('.agenda-event, .agenda-magazine').data({height: 24});
      $('.agenda-type').addClass('hni-grid').data({paddingTop: 1}); // do not remove - nth-child overider.
      $('.page-magazines .agenda-magazine').data({paddingTop: 1});
      $('.agenda-magazine:nth-child(-n+2) > .agenda-type, .agenda-event:nth-child(-n+2) > .agenda-type').data({paddingTop: 0});
      $('.page-magazines .agenda-magazine:nth-child(-n+2)').data({paddingTop: 0});
      $('.footer-col').addClass('hni-grid').data({paddingBottom: 1, marginBottom: 0});
      $('.footer-col:last-child').addClass('hni-grid').data({paddingBottom: 0, marginBottom: 0});
      $('.footer-wrapper .col1:nth-child(2) > .footer-col:nth-child(2)').addClass('hni-grid').data({paddingTop: 0});
      $('#agenda-filter-form').addClass('hni-grid').data({marginTop: 2});
      $('.agenda-header-content').addClass('hni-grid').data({paddingBottom: 0.5});
      $('.page-magazines .agenda-magazine > .agenda-event-body').addClass('hni-grid').data({paddingLeft: 1, paddingRight: 1});
    }
    ...
    if (current_range > 1) { // tablet or desktop
      ...
      $('.container-magazine-homepage-item').addClass('hni-grid').data({height: 3});
      $('.events-next-list').addClass('hni-grid').data({minHeight: 21});
      $('.related-magazines .elenco-section').addClass('hni-grid').data({height: 12});
      $('.page-section-news-overview, .page-section-news-overview .body').data({height: 16});
      ...
      $('.hni-header__vertical-menu ul').removeClass('.hni-grid');
      $('.magazine-homepage-item.left-left').addClass('hni-grid').data({paddingBottom: 0, paddingTop: 0});
      $('.magazine-homepage-cover').addClass('hni-grid').data({paddingBottom: 1, paddingTop: 1});
      $('.magazine-homepage-item.left-right').addClass('hni-grid').data({minHeight: 4, paddingBottom: 0});
      $('#agenda-filter-form').css('display','block');
      $('#edit-when').addClass('hni-grid').data({marginTop: 0, marginBottom: 0});
      $('#edit-type-container').addClass('hni-grid').data({marginBottom: 0});
      $('.magazine-homepage-item.right-right').addClass('hni-grid').data({paddingBottom: 0});
      $('.agenda-group-title').addClass('hni-grid').data({paddingBottom: 1});
      $('.page-magazines .agenda-magazine .event-body-teaser').addClass('hni-grid').data({height: 4.5}).css('height','');
      $('.magazine-header-title').addClass('hni-grid').data({paddingBottom: 0});
      $('.page-section-event-general-information .label-block-border').addClass('hni-grid').removeData("height");
      $('.page-section-magazine-general-information .first .label-block-border').addClass('hni-grid').data({height: 6});
      $('.page-section-magazine-general-information .first .label-block-border').addClass('hni-grid').data({minHeight: 6});
      $('.page-section-magazine-general-information .second .label-block-border').addClass('hni-grid').data({height: 10});
      $('.page-section-magazine-general-information .second .label-block-border').addClass('hni-grid').data({minHeight: 10});
      $('.label-ticket').addClass('hni-grid').data({marginTop: 1});
      $('.page-node.not-front .region-content').addClass('hni-grid').data({paddingTop: 0});
      $('.page-node.not-front #main-wrapper #block-locale-language--2').addClass('hni-grid').data({paddingTop: 1});
      $('.press-item').addClass('hni-grid').data({paddingBottom: 3});
      ...
      $('.related-magazines .elenco-section').addClass('hni-grid').data({height: 16});
      ...
      $('.hni-header__item-menu ul li, .hni-header__item-event, .hni-header__item-news').removeData( "height" ).css('height', 'auto');
      $('.agenda-type').data({paddingTop: 1});
      $('.page-magazines .agenda-magazine').data({paddingTop: 1});
      $('.agenda-magazine.agenda-event:nth-child(-n+2) > .agenda-type').data({paddingTop: 0});
      $('.page-magazines .agenda-magazine:nth-child(-n+2)').data({paddingTop: 1});
      $('.agenda-event, .agenda-magazine').removeData( "height" ).css('height', 'auto');
      $('.page-agenda .agenda-event .event-body-teaser').removeClass('hni-grid').removeData("height").css('height','auto');
      $('.magazine-homepage-item.left-left').removeData( "height" ).css('height', 'auto').addClass('hni-grid').data({paddingBottom: 0, marginBottom: 0});
      $('.magazine-homepage-item.left-right').addClass('hni-grid').data({paddingBottom: 0});
      $('.magazine-homepage-cover').addClass('hni-grid').data({paddingBottom: 1, paddingTop: 1});
      $('.hni-header__vertical-menu').addClass('hni-grid').data({paddingBottom: 1});
      $('.hni-header-home').addClass('hni-grid').data({marginBottom: 0});
      $('.label-ticket').addClass('hni-grid').data({marginTop: 0});
      $('#edit-type, #edit-location, #edit-when-date-group').addClass('hni-grid').data({paddingTop: 0.5});
      $('#edit-location-container').addClass('hni-grid').data({paddingBottom: 0});
      $('.agenda-header').addClass('hni-grid').data({paddingTop: 0});
      $('.agenda-header-content').addClass('hni-grid').data({paddingTop: 1, paddingBottom: 1});
      $('#edit-when').addClass('hni-grid').data({marginTop: 1.5, marginBottom: 0, paddingBottom: 0});
      $('#edit-type-container').addClass('hni-grid').data({marginBottom: 1});
      $('#agenda-filter-form').addClass('hni-grid').data({marginTop: 2}).css('display','none');
      $('.magazine-homepage-cover .magazine-homepage-item.left-right > a.magazine-homepage-cover-link').data({paddingTop: 0.5});
      $('.page-magazines .agenda-magazine > .agenda-event-body').addClass('hni-grid').data({paddingLeft: 1, paddingRight: 1});
      $('.page-magazines .agenda-magazine .event-body-teaser').addClass('hni-grid').data({height: 4.5});
      $('.magazine-header-title').addClass('hni-grid').data({paddingBottom: 1});
      $('.label-block-border').addClass('hni-grid').removeData("height").css('height', 'auto');
      $('.label-block-border').addClass('hni-grid').removeData("min-height");
      $('.page-section-magazine-general-information .label-block-border').addClass('hni-grid').data({minHeight: 0});
      $('.page-node.not-front #main-wrapper #block-locale-language--2').addClass('hni-grid').data({paddingTop: 2});

      if (current_range > 0) { // >= 480
        $('.magazine-homepage-item.left-left').addClass('hni-grid').data({minHeight: 2});
        $('.magazine-homepage-item.left-right').addClass('hni-grid').data({paddingBottom: 1});
        $('.magazine-homepage-item.right-left').addClass('hni-grid').data({height: 4});
        $('.magazine-homepage-item.right-right').addClass('hni-grid').data({height: 1, paddingBottom: 0});
        $('.container-magazine-homepage-item').addClass('hni-grid').data({height: 4});
        $('.programme-tracks .agenda-magazine-cover-image-link').data({minHeight: 0, height: 14});
        $('#footer .footer-wrapper .col1:nth-child(2)').addClass('hni-grid').data({marginBottom: 1});
        $('.footer-col:last-child').addClass('hni-grid').data({paddingBottom: 0, marginBottom: 0});
        $('.hni-header-home').addClass('hni-grid').data({marginTop: 0.5});
        $('.agenda-responsive').addClass('hni-grid').data({paddingTop: 1});
        $('#edit-when').addClass('hni-grid').data({marginTop: 0, marginBottom: 0});
        $('#edit-type-container').addClass('hni-grid').data({marginBottom: 0});
        $('#agenda-filter-form').addClass('hni-grid').data({marginTop: 1});
        $('.agenda-group-title').addClass('hni-grid').data({paddingBottom: 0});
        $('.page-node.not-front .region-content').addClass('hni-grid').data({paddingTop: 1});
        $('.press-item').addClass('hni-grid').data({paddingBottom: 4});

      } else { // < 480
        $('.magazine-homepage-item.left-left').addClass('hni-grid').data({minHeight: 2});
        $('.magazine-homepage-item.left-right').addClass('hni-grid').data({paddingBottom: 1});
        $('.magazine-homepage-item.right-left').addClass('hni-grid').data({height: 6, paddingTop: 1});
        $('.magazine-homepage-item.right-right').addClass('hni-grid').data({height: 2, paddingTop: 1, marginBottom: 1});
        $('.container-magazine-homepage-item').addClass('hni-grid').data({height: 6});
        $('.footer-col').addClass('hni-grid').data({paddingBottom: 1, marginBottom: 0});
        $('.footer-wrapper').addClass('hni-grid').data({paddingTop: 1});
        $('#footer .footer-wrapper .col1:nth-child(2)').removeClass('hni-grid').data({marginBottom: 0});
        $('.hni-header-home').addClass('hni-grid').data({marginTop: 1});
        $('.page-node.not-front .region-content').addClass('hni-grid').data({paddingTop: 4});
        $('.press-item').addClass('hni-grid').data({paddingBottom: 6.5});
        $('.page-section-news-overview .elenco-section, .page-section-news-overview .body').data({height: 24});
      }
    }

    // Really specific ranges & responsive fixes
    if (Drupal.get_viewport_size().width > 1200) {
      $('.programme-tracks .agenda-magazine-cover-image-link').data({minHeight: 0, height: 12});
    }
    if (Drupal.get_viewport_size().width > 1200 && Drupal.get_viewport_size().width < 1300 ) {
    }
    if (Drupal.get_viewport_size().width < 767) {
      $('fieldset.form-wrapper, .agenda-content').removeData( "height" ).css('height', 'auto');
      $('.agenda-header, .agenda-header-content').removeData( "height" ).css('height', 'auto');

    }
    if (Drupal.get_viewport_size().width > 641) {
      $('#edit-type-container').addClass('hni-grid').attr('data-margin-top', 0);
    }
    if (Drupal.get_viewport_size().width < 640) {
      $('.agenda-header, .agenda-header-content').removeData( "height" ).css('height', 'auto');
    }
  ...
  Drupal.behaviors.hniGrid = {
    attach: function(context) {
      ...
      $('#content .h1, #content h2, #content .h2, #content p, #content ul, #content ol, #content blockquote, .block-menu-block, div.messages, .slider-inline, .hni-item, .field-name-iframe-caption').addClass('hni-grid').attr('data-margin-bottom', 1);
      $('.also-this-evening-content p').attr('data-margin-bottom', 0.3);
      ...
      // $('#main-wrapper #block-locale-language--2').addClass('hni-grid').attr('data-padding-top', 1);
      $('.page-magazines #block-locale-language--2, .page-agenda #block-locale-language--2').addClass('hni-grid').attr('data-padding-top', 2);
      $('.field-item-page-section-text, .field-item-page-section-redirect, .elenco-section-item .button').addClass('hni-grid').attr('data-margin-top',1).attr('data-margin-bottom',1);
      $('.homepage .event-inner-wrapper').addClass('hni-grid').attr('data-padding-bottom',1); // .field-item-page-section-text
      ...
      $('.webform-client-form').addClass('hni-grid').attr('data-padding-top', 1);
      $('.page-section-elenco .field-name-excerpt h2').addClass('fullwidth h5 elenco item surtitle').attr('data-margin-bottom', 0);
      $('.page-section-elenco .field-name-excerpt h3').addClass('h4 elenco item maintitle').attr('data-margin-bottom', 0);
      $('.page-section-elenco .field-name-excerpt p').addClass('h4 hni-grid');
      $('.field-item-page-section-elenco').addClass('hni-grid').attr('data-margin-top', 1);
      $('.page-section-excerpt').addClass('hni-grid').attr('data-padding-top', 1);
      $('.elenco-section .button-wrapper').addClass('hni-grid').attr('data-margin-bottom', 0.5);
      $('.share-social').removeClass('hni-grid').attr('data-margin-bottom', 0);
      $('.page-section-image-grid').addClass('hni-grid').attr('data-margin-top', 1);
      ...
      $('.events-group-title').attr('data-margin-bottom', 0);
      $('.page-section-news-overview .elenco-section').data({paddingTop: 0});
      $('.elenco-section.first').data({paddingTop: 0});
      $('.elenco-section.last').data({paddingBottom: 0});
      $('.page-section-news-overview .elenco-section.last').data({paddingBottom: 1});
      ...
      $('#block-corporate-main-menu .menu.menu').addClass('hni-grid').attr('data-padding-bottom', 0.5);
      ...
      $('.programme-tracks .agenda-magazine-cover-image-link').addClass('hni-grid').attr('data-padding-bottom', 1);
      ...
      $('.node-news .page-section-image-gallery').addClass('hni-grid').attr('data-margin-top', 1);
      $('.page-section-image-gallery.inline').addClass('hni-grid').attr('data-padding-top', 1);
      ...
      $('.homepage-announcement-wrapper .h3').addClass('hni-grid').attr('data-margin-bottom', 1);
      $('.field-name-iframe-caption').addClass('hni-grid').attr('data-padding-top', 0.5);
      $('.sponsor-list').removeClass('hni-grid');
      $('.hni-header__vertical-menu ul').removeClass('hni-grid').attr('data-margin-bottom', 0);
      $('.hni-header__item-news--type').attr('data-margin-bottom', 0);
      $('#agenda-filter-form').addClass('hni-grid');
      $('.agenda-datepicker-label').addClass('h4');
      $('.magazine-homepage-cover .magazine-homepage-item.left-right > a.magazine-homepage-cover-link').attr('data-padding-top', 0).attr('data-padding-bottom', 0.5);
      $('.magazine-homepage-cover-link').attr('data-padding-top', 0);
      $('.domain-hni.not-front.page-agenda  #footer').addClass('hni-grid').attr('data-margin-top', 1);
      $('#agenda-filter-form .form-item-when-date-exact img').addClass('hni-grid').data({top: 1});
      $('#agenda-filter-form .agenda-datepicker-label').addClass('hni-grid').data({paddingTop: 1});
      $('#block-magazine-sharing').addClass('hni-grid').data({paddingBottom: 1});
      $('.menu-block-wrapper > .menu > li:nth-child(5) > .menu').addClass('hni-grid').data({paddingBottom: 0});
      $('.magazine-header-links .item-list > h3, .magazine-header-links .item-list > ul li:not(:last-child)').addClass('hni-grid').data({paddingBottom: 0.5});
      $('.magazine-header-links .item-list > ul').addClass('hni-grid').data({padding: 1});
      $('.page-magazines .agenda-group').addClass('hni-grid').data({marginBottom: 1});
      $('.related-magazines .elenco-section.last').addClass('hni-grid').data({marginBottom: 0});
      $('.field-item-page-section-news-overview').addClass('hni-grid').data({paddingTop: 1});
...
(function ($) {
  Drupal.behaviors.coverPosition = {
    attach: function(context) {
      ...
      // header scrolled down on magazine cover -- hni2016 remove header js bits
  ...
  Drupal.behaviors.hniHeader = {
    attach: function(context) {
      $('.hni-header', context).once('hni-header', function() {
        ...
        if( $body.hasClass('domain-hni') && $body.hasClass('not-front') && !$('body').hasClass('hni-header-expand')) {
        ...
        if( $body.hasClass('domain-hni') && $body.hasClass('not-front') && $body.hasClass('page-node') || $body.hasClass('page-agenda') || $body.hasClass('page-user') || $body.hasClass('page-search') || $body.hasClass('page-magazines')) {
        ...
        if( $body.hasClass('domain-hni') && $body.hasClass('not-front') && $body.hasClass('page-node') || $body.hasClass('page-agenda') || $body.hasClass('page-magazines')) {
          $('#header-section').css('display','none');
        }
  ...
  Drupal.behaviors.magazinesIframe = {
    attach: function(context) {
      $('.page-section-iframe', context).once('magazines-iframe', function() {
        ...
        if (!touch || !$('.field-name-image', $this).length) {
          activate_iframe();
        } else {
...
(function ($) {
	...
	// agenda responsive menu
	 Drupal.behaviors.agendaresp = {
	    attach: function(context) {
	      $('.agenda-header', context).once('.agenda-header', function() {
				$( ".agenda-responsive" ).click(function () {
				  if ( $( "#agenda-filter-form:first" ).is( ":hidden" ) ) {
				    $( "#agenda-filter-form" ).slideDown( "slow");
				    $(".agenda-responsive").toggleClass('ar-not-active ar-active');
				  } else {
				    $( "#agenda-filter-form" ).slideUp();
				    $(".agenda-responsive").toggleClass('ar-not-active ar-active');
				  }
				});
			});
	...
	// agenda & webmagazine overview sticky header
	Drupal.behaviors.stickyHeaderAgenda = {
	attach: function(context) {
			$('.agenda-header', context).once('.agenda-header', function() {

	        var stickyheader = function() {
					var headerHeight = $('.agenda-header').height();

			    if (Drupal.get_viewport_size().width > 767) {
			        $('.agenda-content').css({'padding-top': headerHeight});
			    } else {
	...
	// webmagazine dropdown menu
	Drupal.behaviors.webmagazineDropdown = {
	attach: function(context) {
			$('.magazine-header-links', context).once('.magazine-header-links', function() {

				var menuButton = $('.magazine-header-links h3');
				var dropdownMenu = $('.magazine-header-links ul');

				$(menuButton).on('click', function() {
					menuButton.removeClass('active');
					dropdownMenu.not(
						$(this).siblings(dropdownMenu).fadeToggle(),
	...
	// Add class for webmagazines overview
	Drupal.behaviors.webmagazineClass = {
	attach: function(context) {
			$('.magazine-header-content', context).once('.magazine-header-content', function() {

				var filterCheck = $('#magazine-filter');

				if (filterCheck.hasClass('magazine-header-content')) {
					$('#agenda-items').addClass('magazine-group-content').removeClass('agenda-group-content');

					if (Drupal.get_viewport_size().width > 767) {
						$('.magazine-group-content > .agenda-magazine .event-body-teaser').addClass('hni-grid').data({height: 4.5}).css('height','');
					}
				}
...
;
(function ($) {
  // related blocks text handling
  Drupal.behaviors.relatedBlock = {
    attach: function(context) {
      $('#block-magazine-related-magazines', context).once('block-magazine-related-magazines', function() {
        var textBlock = $('.text');
        var related_resize = function() { $(".related-magazines .related__item").each(function(){
          var $this = $(this);
          ...
          $this.find('.text')
               .height( containerHeight - (titleHeight + imageHeight) )
               .trigger("update.dot");
          });
        };

        $(window).on('load resize', related_resize);
        related_resize();
      });
    }
  };
...
;
(function ($) {
  Drupal.behaviors.newsletterToggler = {
    attach: function(context) {
      $('.field-item-page-section-newsletter--small .js--newsletter-email', context).once('newsletter-toggler', function() {
        $self = $(this);

        $self.on('focus', function(e) {
          e.preventDefault();
          $('.form-group--newsletter').removeClass('form-group--small');
        });
      });
    }
  };
...
(function ($) {
  // related blocks text handling
  Drupal.behaviors.creditsLink = {
    attach: function(context) {
      $('.credits-link', context).once('credits-toggle-link', function() {
        var $self = $(this);

        $self.on('click', function(e) {
          e.preventDefault();
          $self.next().toggleClass('visually-hidden');
        });
      });
    }
  };
...
(function ($) {
  // corporate header
  Drupal.behaviors.general_information = {
    attach: function(context) {
      $('.general-information-equalize', context).once('general-information-equalize', function() {
        var $this = $(this);

        var setup = function() {
          var height = 0;

          $('.label-block', $this).each(function() {
            $(this).css('height', 'auto');
            height = Math.max(height, $(this).outerHeight());
          });

          if (height) {
            $('.label-block', $this).css('height', '' + height + 'px');
          }
        };
